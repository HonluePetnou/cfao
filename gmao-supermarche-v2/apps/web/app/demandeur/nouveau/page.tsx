...
export default function NouveauTicketDemandeurPage() {
  ...
  // Le site est toujours choisi explicitement, même pour un demandeur en
  // principe rattaché à un seul supermarché : ça affiche clairement pour
  // quel site le ticket part, et ça couvre le cas d'un profil sans site
  // assigné, plutôt que de deviner silencieusement.
  ...
  const handleSiteChange = (smId: string) => {
    ...
    if (typeof window !== "undefined") sessionStorage.setItem("gmao_current_supermarket", smId);
    loadForSite(smId);
  };
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        // Pré-sélectionne le site du profil du demandeur (ou celui de la
        // session) — reste un vrai choix visible et modifiable, pas une
        // valeur imposée en silence.
        const preselect = u.supermarketId || sessionStorage.getItem("gmao_current_supermarket") || "";
        if (preselect && sms.some((s: any) => s.id === preselect)) {
          setSupermarketId(preselect);
          await loadForSite(preselect);
        }
      } catch {}
      finally {
        setLoading(false);
  ...
  // Un équipement créé pendant que cette page était déjà ouverte (par un
  // admin, dans un autre onglet) doit apparaître sans recharger la page à
  // la main : on rafraîchit la liste dès que l'onglet redevient actif.
  useEffect(() => {
    const refresh = () => {
      if (document.visibilityState !== "visible" || !supermarketId) return;
      api.getEquipments({ supermarketId }).then(setEquipments).catch(() => {});
    };
  ...
  const filteredEquipments = useMemo(() => {
    ...
    let list = [...equipments];
    // Un équipement sans corps d'état / localisation renseigné (données existantes
    // créées avant l'ajout de ces champs) ne doit jamais être masqué par le filtre —
    // seulement les équipements qui ont une valeur différente de celle sélectionnée.
    if (corpsEtat) {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <form onSubmit={handleSubmit} className="max-w-lg mx-auto px-4 py-5 pb-32 space-y-4">
        ...
        {/* ── Section 5: Maintenancier affecté ── */}
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="px-4 pt-4 pb-1 flex items-center gap-2">
